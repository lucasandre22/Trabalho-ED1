...
void monta_caminho(clista *vetor_lista_localidades, int tam_vet_id, char** vet_id)
{
    ...
    for(k=0; k<tam_vet_id; k++)
    {
        ...
        while(vetor_lista_localidades[k].cabesalho!=NULL)
        {
            ...
            {
                /*Atualiza o vetor de ponteiros, porque pode ser que uma localidade exista no segundo caminho
                porém não exista no primeiro, como juntamos os caminhos então atualizamos quais localidades
                que podemos fazer o merge entre si*/
